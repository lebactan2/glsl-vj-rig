void main() {
    ...
    if (p.y > 0.4) {
        // --- Top Section: Lime Green UI ---
        col = vec3(0.55, 0.82, 0.35);
        
        // Abstract blue text "Weirdcore" on left
        if (p.x > -1.2 && p.x < -0.4 && p.y > 0.6 && p.y < 0.75) {
            float letters = fract(p.x * 5.0);
            if (letters < 0.7 && fract(p.x * 20.0 + p.y*10.0 + iTime*2.0) > 0.2) { // Animated scanning lines
                col = vec3(0.1, 0.2, 0.7);
            }
        }
        
        // "#18" in middle (pulsing)
        if (abs(p.x) < 0.1 && p.y > 0.65 && p.y < 0.7) {
            col = mix(vec3(0.2, 0.3, 0.5), vec3(0.1, 0.2, 0.8), sin(iTime*5.0)*0.5+0.5);
        }
        
        // "Synesthesia" logo on right (blue fluid shapes animated)
        ...
        if (djDist < 0.0) {
            ...
            col += vec3(0.1, 0.15, 0.1) * (0.5 + 0.5 * sin(p.y * 50.0)) * smoothstep(-0.5, -0.8, p.y);
        }
        
        vec2 screenL = p - vec2(-0.6, -0.6);
        float sL = max(abs(screenL.x) - 0.15, abs(screenL.y) - 0.1);
        if (sL < 0.0) {
            col = vec3(0.2, 0.8, 0.4);
            // Screen contents animated
            if (fract(screenL.x*10.0 - iTime) < 0.5 && screenL.y > 0.0) col = vec3(0.3, 0.9, 0.5);
            if (abs(screenL.y) > 0.08 || abs(screenL.x) > 0.13) col = vec3(0.8);
        }
        ...
        if (sR < 0.0) {
            ...
            if (fract(screenR.x * 20.0 + iTime*2.0) < 0.5 && screenR.y < 0.0) col = vec3(0.3, 0.3, 0.8);
        }
