void main() {
    ...
    // Background: Store interior with checkered floor
    vec3 col = vec3(0.85, 0.85, 0.85); // Wall color
    
    // Top wall section (brick texture)
    if (p.y > 0.6) {
        ...
        if (mortar > 0.0) col = vec3(0.7, 0.65, 0.6); // Mortar color
    }
    
    // Clothing Racks in Background
    if (p.y > 0.0 && p.y < 0.6) {
        col = vec3(0.15); // Dark background depth
        
        // Rack structure (black metal lines)
        ...
        if (length(cp * vec2(0.5, 1.0)) < 0.4) {
            // Random colors for shirts
            float rnd = fract(floor(p.x * 15.0) * 12.3 + floor(p.y * 5.0) * 45.6);
            if (rnd < 0.2) col = vec3(0.8, 0.2, 0.2); // Red
            else if (rnd < 0.4) col = vec3(0.2, 0.3, 0.8); // Blue
            else if (rnd < 0.6) col = vec3(0.9, 0.9, 0.2); // Yellow
            else col = vec3(0.9); // White
            
            // Shirt folds
            col *= 0.8 + 0.2 * sin(cp.x * 20.0);
        }
        
        // Shelf with folded clothes on bottom left
        if (p.x < -0.4 && p.y < 0.2) {
             col = vec3(0.4, 0.4, 0.45); // Grey stacks
             float folds = fract(p.y * 40.0);
             if (folds < 0.1) col = vec3(0.2); // Shadow between items
        }
    }
    ...
    if (p.y <= 0.0) {
        ...
        // Fade floor to black in distance (depth shading)
        col *= smoothstep(-0.2, 0.2, -p.y);
    }
    
    // Main Subject (Person taking mirror selfie)
    ...
    float armR1 = length(max(abs(torsoP - vec2(0.35, 0.1)) - vec2(0.05, 0.15), 0.0)) - 0.02; // Upper arm
    float armR2 = length(max(abs(torsoP - vec2(0.2, 0.3)) - vec2(0.15, 0.04), 0.0)) - 0.02; // Lower arm crossing chest
    
    if (torso < 0.0 || armL < 0.0 || armR1 < 0.0 || armR2 < 0.0) {
        
        // Shirt Base Color (White/Light Grey)
        col = vec3(0.9, 0.9, 0.95);
        
        // Vertical Stripes Pattern
        // Project coordinates for clothing to make stripes flow
        vec2 clothUV = torsoP;
        if (armL < 0.0 && torso > 0.0) clothUV.x -= 0.1; // Offset stripes on arms
        
        // Animation: Stripes moving/flowing slightly to simulate fabric shift
        float stripeAnim = sin(clothUV.y * 5.0 + iTime * 2.0) * 0.02;
        
        float stripes = step(0.5, fract((clothUV.x + stripeAnim) * 12.0));
        
        // Stripe color (Dark Grey/Blueish)
        ...
        if (pocket < 0.01) {
            ...
            if (pocket > 0.005) col = vec3(0.2);
                ...
                if (snoopyHead < 0.0) col = vec3(0.95); // White head
                if (snoopyEar < 0.0 || snoopyNose < 0.0) col = vec3(0.1); // Black ear/nose
            }
        }
        ...
        if (seam < 0.01 && torsoP.y < 0.35) {
            col = mix(col, vec3(0.2), 0.3); // Shadow for seam
            // Buttons
            if (fract(torsoP.y * 5.0) < 0.1) col = vec3(0.1); // Black buttons
        }
    ...
    float bareArmL = length(max(abs(torsoP - vec2(-0.35, -0.35)) - vec2(0.03, 0.15), 0.0)) - 0.02; // Left forearm
    float handL = length(max(abs(torsoP - vec2(-0.2, -0.4)) - vec2(0.05, 0.04), 0.0)) - 0.01; // Left hand doing peace sign
    float handR = length(max(abs(torsoP - vec2(0.05, 0.35)) - vec2(0.04, 0.05), 0.0)) - 0.02; // Right hand holding phone
    ...
    // Shorts (Matching Striped pattern)
    ...
    if (legL < 0.0 || legR < 0.0) {
        col = vec3(0.9, 0.7, 0.6); // Skin tone
        col *= 0.7 + 0.3 * sin(shortsP.x * 40.0); // Shading
    }

    // Head
    float head = length(p - vec2(0.0, 0.55)) - 0.12;
    if (head < 0.0) {
        col = vec3(0.9, 0.7, 0.6); // Skin
    ...
    if (phone < 0.0) {
        col = vec3(0.15); // Black phone case
        // Camera lens cluster
