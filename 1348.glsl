void main() {
    ...
    vec3 col = vec3(0.9, 0.9, 0.92);
    
    // Animated Volumetric God Rays from left window
    if (p.x < 0.5) {
        float rayAngle = atan(p.y - 0.6, p.x - (-0.8));
        float rays = sin(rayAngle * 20.0 + iTime * 0.5) * sin(rayAngle * 10.0 - iTime * 0.2);
        rays = max(0.0, rays);
        float mask = smoothstep(1.0, 0.0, length(p - vec2(-0.8, 0.6))); // fade out
        col += vec3(0.2, 0.25, 0.3) * rays * mask;
    }

    // Pillar with Dragon Relief
    float column = abs(p.x - 0.7);
    if (column < 0.2) {
        col = vec3(0.25, 0.5, 0.7); 
        col *= 0.5 + 0.5 * cos((p.x - 0.7) * 7.85); // Cylinder shading
        
        // Deep Carved Dragon Scales / Clouds
        float rY = fract(p.y * 5.0 + sin(p.x * 10.0)*0.2);
        float rX = (p.x - 0.7) * 5.0;
        float cloud = sin(rY * 10.0 + rX * 10.0) * cos(rY * 10.0 - rX * 10.0);
        
        if (cloud > 0.3) {
            col = mix(col, vec3(0.8, 0.2, 0.2), 0.7); // Red relief
            if (cloud > 0.7) col = mix(col, vec3(0.9, 0.8, 0.2), 0.8); // Gold trim
            col *= 1.0 - 0.3 * smoothstep(0.3, 0.4, cloud); // self shadow
        }
    }

    // The Headpiece (Detailed Wood)
    vec2 hpP = p - vec2(0.0, 0.65);
    float hpCurve = length(max(abs(hpP - vec2(0.0, 0.1)) - vec2(0.15, 0.05), 0.0)) - 0.04;
    // Carve out bottom to make horns
    hpCurve = max(hpCurve, -(length(hpP - vec2(0.0, 0.2)) - 0.15));
    
    if (hpCurve < 0.0) {
        col = vec3(0.4, 0.2, 0.1); 
        // Animated Wood grain glimmer
        float grain = sin(hpP.x * 80.0 + hpP.y * 30.0 + sin(hpP.x * 20.0)*5.0);
        col *= 0.7 + 0.3 * grain;
        col += vec3(0.1) * exp(-50.0 * pow(hpP.y - 0.12, 2.0)); // Specular highlight
    }
    ...
    if (veilTop < 0.0 && p.y > -1.0 && p.x > drapeL && p.x < drapeR) {
        if (faceMask < 0.0) {
            ...
            float blink = smoothstep(0.95, 1.0, sin(iTime * 2.0)); // rapid blink mask
            if (min(eyeL, eyeR) < 0.015 && abs(p.y - 0.45) > blink * 0.015) {
                col = vec3(0.1); // Dark eyes
            }
            // Mouth
            if (length(max(abs(p - vec2(0.0, 0.32)) - vec2(0.03, 0.005), 0.0)) < 0.01) col = vec3(0.3, 0.1, 0.1);
            
        } else {
            // Animated Billowing Veil Fabric
            col = vec3(0.95, 0.95, 1.0); 
            float sway = sin(p.y * 5.0 - iTime * 2.0) * 0.05; // fabric waving
            ...
            // The "Divine Eye" Emblem
            vec2 embP = p - vec2(0.0, -0.3);
            
            // Emblem floating/breathing animation
            ...
            if (handL < 0.0 || handR < 0.0) {
                col = vec3(0.8, 0.6, 0.45); 
                if (fract(p.x * 40.0) < 0.1) col *= 0.8; // fingers
            }
        }
    }
