void main() {
    ...
    // Background: Mosaic wall (glassy tiles)
    vec3 col = vec3(0.0);
    ...
    vec3 tileCol = vec3(0.6 + 0.3*sparkle, 0.65 + 0.25*sparkle, 0.7 + 0.2*sparkle); // slight tint
    ...
    // Inside the frame
    if (frameInner < 0.0) {
        // Base wall color inside frame
        col = vec3(0.85, 0.85, 0.85);
        
        // Center wooden frame structure
        float centerHoleOuter = max(abs(p.x) - 0.4, abs(p.y) - 0.5);
        float centerHoleInner = max(abs(p.x) - 0.35, abs(p.y) - 0.45);
        
        if (centerHoleOuter < 0.0) {
            if (centerHoleInner > 0.0) {
                // The wooden frame itself
                col = vec3(0.15); // Dark border
                // Wood grain effect
                if (fract(p.x*20.0 + sin(p.y*10.0)) < 0.2) col *= 0.8;
            } else {
                // Inside the wooden frame (exposed wall and wires)
                col = vec3(0.6, 0.55, 0.5); // Grungy wall
                // Wall texture
            ...
            // Quilted leather padding around the center
            vec2 diaP = p;
            float diaSize = 0.35;
            
            // Shift for diamond pattern
            ...
            float pillow = 1.0 - max(d1, d2); // Diamond shape
            
            // Silver shiny leather
            ...
            // Adjust button positions to match diamond corners
            if (length(fDia) < 0.04) {
                // Silver button
