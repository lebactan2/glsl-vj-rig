void main() {
    ...
    vec3 col = vec3(0.55, 0.55, 0.56) + concreteNoise * 0.06 - 0.03;
    
    #define segment(p, a, b) length(p - a - (b - a) * clamp(dot(p - a, b - a) / dot(b - a, b - a), 0.0, 1.0))

    // Highly detailed calligraphic "福" (Fortune) with varying stroke thickness
    float charShape = 1.0;
    
    // Left radical - sweeping strokes
    charShape = min(charShape, segment(p, vec2(-0.4, 0.3), vec2(-0.5, -0.3)) - p.y * 0.02); // Tapering
    ...
    float mask = 1.0 - smoothstep(0.04, 0.06, charShape);
    
    // Cracked glaze Voronoi
    vec2 shardP = p * 20.0;
    vec2 g = floor(shardP);
    vec2 f = fract(shardP);
    float minDist = 1.0;
    ...
    // Grout lines + normal mapping for beveled shards
    ...
    // Mix blue and white shards randomly
    ...
    // Add cracked glaze texture inside shards
