void main() {
    ...
    vec3 col = vec3(0.05); // Dark signboard
    ...
    if (abs(p.x) < signW && abs(p.y) < signH) {
        ...
        // 2. Text: "NƯỚC MÍA" (Red, large, center)
        ...
        if (d2 < 0.02) {
            col = vec3(0.9, 0.1, 0.15); // bright red
            // Add slight drop shadow
            if (d2 > 0.01) col = vec3(1.0, 0.8, 0.8); // outline
        }
        
        // Detailed Sugarcane stalks
        for (float i = 0.0; i < 4.0; i++) {
            ...
            if (stalk < 0.025) {
                ...
                if (seg > 0.9) stalkCol = vec3(0.3, 0.4, 0.1); // joint ring
                
                col = stalkCol;
            }
        }
        
        // Detailed Sugarcane leaves swooping
        vec2 lp = p - vec2(-0.8, -0.6);
        float leaf = max(lp.y - lp.x * lp.x * 2.0, -lp.y - 0.05 + lp.x * lp.x * 1.5);
        if (leaf < 0.0 && lp.x > 0.0 && lp.x < 1.0) {
            col = vec3(0.2, 0.8, 0.2); // vibrant green
            // Vein
