void main() {
    ...
    // Animate a soft light panning across
    float lightMove = sin(iTime * 0.5) * 0.5 + 0.5;
    
    vec3 col = vec3(0.8, 0.5, 0.3); // base brick color
    // Add detail to brick
    ...
    if (abs(p.x) < 0.8) {
        col = vec3(0.1); // behind slats
        
        // Window slats with animation
        ...
        if (slats > 0.3) {
            ...
            // Animated light reflection on slats
