void main() {
    ...
    vec3 col = vec3(0.1, 0.15, 0.15) * (1.0 - length(p)*0.3); // Dark background
    
    vec3 glazeColor = vec3(0.65, 0.75, 0.75); // Light blue/grey glaze
    vec3 edgeColor = vec3(0.6, 0.45, 0.35); // Exposed clay edge
        ...
        if (p.y > roofCurve) {
            ...
            if (abs(p.x) > 0.35 && abs(p.x) < 0.6) {
                ...
                if (min(min(d1, d2), min(d3, d4)) > 0.15) {
                    // Inside the holes
                    // Animate shadows inside based on lightPos
            ...
            // Center open shrine area
            if (abs(p.x) < 0.25 && p.y < 0.1 && p.y > -0.3) {
                // Dark interior
                col = vec3(0.1, 0.1, 0.12);
                
                // Add animated light shaft/shadow inside
                ...
                if (abs(p.x + 0.08) < 0.03 && p.y > -0.25 && p.y < -0.05) {
                    col = glazeColor * 0.6; 
                    if (p.y > -0.1) col *= 1.2; // Head
                }
                // Right figure
                if (abs(p.x - 0.1) < 0.035 && p.y > -0.25 && p.y < -0.02) {
                    col = glazeColor * 0.65;
                    if (p.y > -0.08) col *= 1.2; // Head
                }
            }
            
            // Decorative Arch above center
            ...
            if (p.y > -0.45 && p.y < -0.3) {
                ...
                if (fract(railP.x) > 0.8 || fract(railP.y) > 0.8) {
                    ...
                    if (abs(fract(railP.x)-0.4) + abs(fract(railP.y)-0.4) < 0.2) col = vec3(0.05); // Holes
                }
            }
            
            // Glaze cracks and weathering
            ...
            vec3 normal = vec3(0.0, 0.0, 1.0); // Simplified flat normal
