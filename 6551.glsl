void main() {
    ...
    if (abs(p.x - winCenter.x) < winSize.x && abs(p.y - winCenter.y) < winSize.y) {
        ...
        if (abs(p.x - winCenter.x) > winSize.x - 0.02 || abs(p.y - winCenter.y) > winSize.y - 0.02) {
            ...
            // Scrolling/shifting grille pattern animation
    ...
    if (p.x > 0.8) {
        ...
        if (p.y < -0.1 + roofCurve*2.0 && p.y > -0.5) {
            ...
            if (p.y > -0.2 + roofCurve*2.0) {
                ...
                float scallops = sin(p.x * 30.0) * sin(p.y * 20.0 + iTime*2.0); // Flowing tiles
        ...
        if (length(dragP) < 0.3) {
            ...
            if (length(dragP) < dragShape) {
                ...
                float spec = pow(max(0.0, sin(dragP.x * 20.0 + dragP.y * 20.0 + iTime*4.0)), 4.0); // Shimmering gold
                col += vec3(spec);
