...
void main() {
    ...
    float petals = cos(ma * 12.0000 + r * 6.4079 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 19.6596) *
                    sin((k.x - k.y) * 25.3972);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.4725 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 7.7376 + t * 1.8 + petals),
        cos(k.x * 9.5799 - t * 1.6 + ringWave)
    ) * 0.0138;
    ...
    vec3 c1 = vec3(0.1843, 0.1922, 0.2039);
    vec3 c2 = vec3(0.6314, 0.6392, 0.6588);
    vec3 c3 = vec3(0.5176, 0.5255, 0.5412);
    vec3 c4 = vec3(0.3333, 0.3412, 0.3529);
    vec3 c5 = vec3(0.3005, 0.5055, 0.1686);
    ...
    float edge = length(photo - photoSoft) * 0.6073;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4630) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6163);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2040, 0.0, 0.42));
    col += c5 * bead * 0.0920;
