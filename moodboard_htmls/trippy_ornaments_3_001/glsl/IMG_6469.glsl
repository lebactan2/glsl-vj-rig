...
void main() {
    ...
    float petals = cos(ma * 10.0000 + r * 5.1447 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.1296) *
                    sin((k.x - k.y) * 25.8231);
    float bead = smoothstep(0.86, 0.99, cos(ma * 23.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.0745 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.8354 + t * 1.8 + petals),
        cos(k.x * 9.7421 - t * 1.6 + ringWave)
    ) * 0.0142;
    ...
    vec3 c1 = vec3(0.1529, 0.1294, 0.1569);
    vec3 c2 = vec3(0.5137, 0.5333, 0.5529);
    vec3 c3 = vec3(0.0588, 0.0510, 0.0824);
    vec3 c4 = vec3(0.1255, 0.1137, 0.1373);
    vec3 c5 = vec3(0.2784, 0.2784, 0.3059);
    ...
    float edge = length(photo - photoSoft) * 0.6190;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.2766) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6119);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2396, 0.0, 0.42));
    col += c5 * bead * 0.1098;
