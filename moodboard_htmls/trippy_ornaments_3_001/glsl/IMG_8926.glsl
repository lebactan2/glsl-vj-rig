...
void main() {
    ...
    float petals = cos(ma * 24.0000 + r * 2.8289 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 25.4651) *
                    sin((k.x - k.y) * 22.8492);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 8.0628 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.3178 + t * 1.8 + petals),
        cos(k.x * 8.6092 - t * 1.6 + ringWave)
    ) * 0.0117;
    ...
    vec3 c1 = vec3(0.6824, 0.6039, 0.4627);
    vec3 c2 = vec3(0.6902, 0.4980, 0.3373);
    vec3 c3 = vec3(0.5176, 0.2627, 0.2431);
    vec3 c4 = vec3(0.8471, 0.7686, 0.6353);
    vec3 c5 = vec3(0.5647, 0.4706, 0.3686);
    ...
    float edge = length(photo - photoSoft) * 0.5375;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5417) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.7569);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2878, 0.0, 0.42));
    col += c5 * bead * 0.1339;
