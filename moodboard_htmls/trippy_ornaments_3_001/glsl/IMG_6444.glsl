...
void main() {
    ...
    float petals = cos(ma * 18.0000 + r * 4.0921 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.0597) *
                    sin((k.x - k.y) * 23.5349);
    float bead = smoothstep(0.86, 0.99, cos(ma * 21.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.3321 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.2488 + t * 1.8 + petals),
        cos(k.x * 8.8704 - t * 1.6 + ringWave)
    ) * 0.0123;
    ...
    vec3 c1 = vec3(0.2667, 0.3098, 0.3490);
    vec3 c2 = vec3(0.5294, 0.6471, 0.7765);
    vec3 c3 = vec3(0.2667, 0.2353, 0.2863);
    vec3 c4 = vec3(0.7294, 0.7255, 0.7098);
    vec3 c5 = vec3(0.4902, 0.5294, 0.5804);
    ...
    float edge = length(photo - photoSoft) * 0.5563;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4443) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.5931);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2531, 0.0, 0.42));
    col += c5 * bead * 0.1165;
