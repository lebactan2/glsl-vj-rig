...
void main() {
    ...
    float petals = cos(ma * 12.0000 + r * 4.8289 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 26.0577) *
                    sin((k.x - k.y) * 23.1852);
    float bead = smoothstep(0.86, 0.99, cos(ma * 24.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.2801 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.5812 + t * 1.8 + petals),
        cos(k.x * 8.7372 - t * 1.6 + ringWave)
    ) * 0.0120;
    ...
    vec3 c1 = vec3(0.2667, 0.2627, 0.1608);
    vec3 c2 = vec3(0.5373, 0.4471, 0.3255);
    vec3 c3 = vec3(0.7725, 0.2706, 0.2824);
    vec3 c4 = vec3(0.6549, 0.5843, 0.4353);
    vec3 c5 = vec3(0.8275, 0.7608, 0.5686);
    ...
    float edge = length(photo - photoSoft) * 0.5467;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4373) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.7448);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2964, 0.0, 0.42));
    col += c5 * bead * 0.1382;
