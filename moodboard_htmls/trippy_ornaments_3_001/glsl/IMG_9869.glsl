...
void main() {
    ...
    float petals = cos(ma * 18.0000 + r * 5.7237 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 18.9108) *
                    sin((k.x - k.y) * 21.6923);
    float bead = smoothstep(0.86, 0.99, cos(ma * 13.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.9857 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 7.4048 + t * 1.8 + petals),
        cos(k.x * 8.1685 - t * 1.6 + ringWave)
    ) * 0.0107;
    ...
    vec3 c1 = vec3(0.5373, 0.5176, 0.5020);
    vec3 c2 = vec3(0.5804, 0.5686, 0.5569);
    vec3 c3 = vec3(0.3569, 0.3451, 0.3373);
    vec3 c4 = vec3(0.3055, 0.4777, 0.1646);
    vec3 c5 = vec3(0.2407, 0.5055, 0.2832);
    ...
    float edge = length(photo - photoSoft) * 0.5059;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5314) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6346);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.1932, 0.0, 0.42));
    col += c5 * bead * 0.0866;
