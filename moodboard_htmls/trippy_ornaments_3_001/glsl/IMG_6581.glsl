...
void main() {
    ...
    float t = iTime * 0.2569;
    vec2 photoP = texUv - 0.5;
    float r = length(p);
    float a = atan(p.y, p.x);
    float segments = 5.0000;
    ...
    float petals = cos(ma * 10.0000 + r * 7.0395 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.2944) *
                    sin((k.x - k.y) * 24.9156);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.9758 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.4642 + t * 1.8 + petals),
        cos(k.x * 9.3964 - t * 1.6 + ringWave)
    ) * 0.0134;
    ...
    vec3 c1 = vec3(0.2706, 0.2980, 0.3098);
    vec3 c2 = vec3(0.1294, 0.1647, 0.1843);
    vec3 c3 = vec3(0.3333, 0.3882, 0.4039);
    vec3 c4 = vec3(0.5882, 0.6118, 0.5922);
    vec3 c5 = vec3(0.7804, 0.7922, 0.7765);
    ...
    float edge = length(photo - photoSoft) * 0.5941;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3968) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6071);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2276, 0.0, 0.42));
    col += c5 * bead * 0.1038;
