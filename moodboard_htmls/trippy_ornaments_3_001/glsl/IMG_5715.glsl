...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 2.2500 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 20.2007) *
                    sin((k.x - k.y) * 21.7590);
    float bead = smoothstep(0.86, 0.99, cos(ma * 12.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.7889 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 7.9781 + t * 1.8 + petals),
        cos(k.x * 8.1939 - t * 1.6 + ringWave)
    ) * 0.0108;
    ...
    vec3 c1 = vec3(0.4902, 0.4784, 0.4235);
    vec3 c2 = vec3(0.5294, 0.5098, 0.4627);
    vec3 c3 = vec3(0.5569, 0.5412, 0.4941);
    vec3 c4 = vec3(0.5059, 0.4980, 0.4549);
    vec3 c5 = vec3(0.3608, 0.3608, 0.3216);
    ...
    float edge = length(photo - photoSoft) * 0.5077;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5052) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6474);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2118, 0.0, 0.42));
    col += c5 * bead * 0.0959;
