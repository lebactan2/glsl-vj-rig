...
void main() {
    ...
    float petals = cos(ma * 20.0000 + r * 4.3553 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.5238) *
                    sin((k.x - k.y) * 21.5287);
    float bead = smoothstep(0.86, 0.99, cos(ma * 20.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.2123 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.0106 + t * 1.8 + petals),
        cos(k.x * 8.1062 - t * 1.6 + ringWave)
    ) * 0.0106;
    ...
    vec3 c1 = vec3(0.4941, 0.4275, 0.3843);
    vec3 c2 = vec3(0.2275, 0.2510, 0.2902);
    vec3 c3 = vec3(0.5333, 0.4667, 0.4196);
    vec3 c4 = vec3(0.5765, 0.5020, 0.4510);
    vec3 c5 = vec3(0.5922, 0.3412, 0.2824);
    ...
    float edge = length(photo - photoSoft) * 0.5014;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4283) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6721);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2453, 0.0, 0.42));
    col += c5 * bead * 0.1127;
