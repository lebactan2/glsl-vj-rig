...
void main() {
    ...
    float petals = cos(ma * 10.0000 + r * 4.6711 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.4391) *
                    sin((k.x - k.y) * 25.3819);
    float bead = smoothstep(0.86, 0.99, cos(ma * 15.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.5977 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.4174 + t * 1.8 + petals),
        cos(k.x * 9.5740 - t * 1.6 + ringWave)
    ) * 0.0138;
    ...
    vec3 c1 = vec3(0.2157, 0.1804, 0.1412);
    vec3 c2 = vec3(0.6431, 0.5725, 0.4353);
    vec3 c3 = vec3(0.7451, 0.6902, 0.5529);
    vec3 c4 = vec3(0.3569, 0.3294, 0.2784);
    vec3 c5 = vec3(0.5020, 0.4353, 0.3216);
    ...
    float edge = length(photo - photoSoft) * 0.6069;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4797) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6967);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2586, 0.0, 0.42));
    col += c5 * bead * 0.1193;
