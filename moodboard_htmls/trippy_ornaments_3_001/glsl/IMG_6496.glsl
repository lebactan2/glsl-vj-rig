...
void main() {
    ...
    float lattice = sin((k.x + k.y) * 22.5027) *
                    sin((k.x - k.y) * 23.9169);
    float bead = smoothstep(0.86, 0.99, cos(ma * 20.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.0566 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.0012 + t * 1.8 + petals),
        cos(k.x * 9.0160 - t * 1.6 + ringWave)
    ) * 0.0126;
    ...
    vec3 c1 = vec3(0.1216, 0.2235, 0.2039);
    vec3 c2 = vec3(0.4353, 0.4549, 0.4196);
    vec3 c3 = vec3(0.5176, 0.5451, 0.4745);
    vec3 c4 = vec3(0.3569, 0.3843, 0.3255);
    vec3 c5 = vec3(0.1804, 0.2667, 0.2196);
    ...
    float edge = length(photo - photoSoft) * 0.5668;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4075) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6250);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2450, 0.0, 0.42));
    col += c5 * bead * 0.1125;
