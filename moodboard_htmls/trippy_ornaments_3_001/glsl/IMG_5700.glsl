...
void main() {
    ...
    float petals = cos(ma * 24.0000 + r * 4.8816 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 25.2500) *
                    sin((k.x - k.y) * 24.1661);
    float bead = smoothstep(0.86, 0.99, cos(ma * 24.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.4329 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.2222 + t * 1.8 + petals),
        cos(k.x * 9.1109 - t * 1.6 + ringWave)
    ) * 0.0128;
    ...
    vec3 c1 = vec3(0.0980, 0.1020, 0.1098);
    vec3 c2 = vec3(0.3020, 0.2078, 0.2353);
    vec3 c3 = vec3(0.5451, 0.4863, 0.3686);
    vec3 c4 = vec3(0.3451, 0.3373, 0.2980);
    vec3 c5 = vec3(0.5059, 0.3725, 0.3137);
    ...
    float edge = length(photo - photoSoft) * 0.5736;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3244) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6804);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2847, 0.0, 0.42));
    col += c5 * bead * 0.1324;
