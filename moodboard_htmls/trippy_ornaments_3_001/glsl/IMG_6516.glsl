...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 2.8289 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 25.0103) *
                    sin((k.x - k.y) * 26.0164);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.5428 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.1157 + t * 1.8 + petals),
        cos(k.x * 9.8158 - t * 1.6 + ringWave)
    ) * 0.0143;
    ...
    vec3 c1 = vec3(0.3922, 0.4078, 0.3765);
    vec3 c2 = vec3(0.1569, 0.1804, 0.1333);
    vec3 c3 = vec3(0.0588, 0.0902, 0.1059);
    vec3 c4 = vec3(0.5529, 0.6118, 0.4118);
    vec3 c5 = vec3(0.2196, 0.2039, 0.1490);
    ...
    float edge = length(photo - photoSoft) * 0.6243;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3390) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6445);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2813, 0.0, 0.42));
    col += c5 * bead * 0.1306;
