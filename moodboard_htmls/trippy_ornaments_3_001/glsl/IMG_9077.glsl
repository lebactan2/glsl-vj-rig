...
void main() {
    ...
    float petals = cos(ma * 10.0000 + r * 4.8289 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.0030) *
                    sin((k.x - k.y) * 24.2620);
    float bead = smoothstep(0.86, 0.99, cos(ma * 12.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.8518 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.7791 + t * 1.8 + petals),
        cos(k.x * 9.1474 - t * 1.6 + ringWave)
    ) * 0.0129;
    ...
    vec3 c2 = vec3(0.1765, 0.1804, 0.2314);
    vec3 c3 = vec3(0.4000, 0.3843, 0.3843);
    vec3 c4 = vec3(0.6078, 0.5725, 0.5255);
    vec3 c5 = vec3(0.7961, 0.7608, 0.7020);
    ...
    float edge = length(photo - photoSoft) * 0.5762;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3802) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6328);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2378, 0.0, 0.42));
    col += c5 * bead * 0.1089;
