...
void main() {
    ...
    float petals = cos(ma * 20.0000 + r * 4.1974 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.0940) *
                    sin((k.x - k.y) * 27.9388);
    float bead = smoothstep(0.86, 0.99, cos(ma * 18.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.8489 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.3751 + t * 1.8 + petals),
        cos(k.x * 10.5481 - t * 1.6 + ringWave)
    ) * 0.0159;
    ...
    vec3 c1 = vec3(0.1765, 0.1294, 0.1255);
    vec3 c2 = vec3(0.6941, 0.6549, 0.5922);
    vec3 c3 = vec3(0.7412, 0.7137, 0.6745);
    vec3 c4 = vec3(0.2784, 0.2353, 0.2549);
    vec3 c5 = vec3(0.9451, 0.9529, 0.9686);
    ...
    float edge = length(photo - photoSoft) * 0.6769;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5132) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6513);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2247, 0.0, 0.42));
    col += c5 * bead * 0.1023;
