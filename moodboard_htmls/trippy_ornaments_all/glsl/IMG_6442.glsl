...
void main() {
    ...
    float petals = cos(ma * 18.0000 + r * 6.7237 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 25.2572) *
                    sin((k.x - k.y) * 26.0706);
    float bead = smoothstep(0.86, 0.99, cos(ma * 22.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.9017 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.2254 + t * 1.8 + petals),
        cos(k.x * 9.8364 - t * 1.6 + ringWave)
    ) * 0.0144;
    ...
    vec3 c1 = vec3(0.6510, 0.6392, 0.6039);
    vec3 c2 = vec3(0.1216, 0.1451, 0.5412);
    vec3 c3 = vec3(0.0745, 0.0863, 0.5020);
    vec3 c4 = vec3(0.5529, 0.5373, 0.5255);
    vec3 c5 = vec3(0.2118, 0.2235, 0.4353);
    ...
    float edge = length(photo - photoSoft) * 0.6257;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3869) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.5412);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2848, 0.0, 0.42));
    col += c5 * bead * 0.1324;
