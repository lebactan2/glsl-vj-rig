...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 3.6184 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.4666) *
                    sin((k.x - k.y) * 24.6590);
    float bead = smoothstep(0.86, 0.99, cos(ma * 25.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.0630 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.5407 + t * 1.8 + petals),
        cos(k.x * 9.2987 - t * 1.6 + ringWave)
    ) * 0.0132;
    ...
    vec3 c1 = vec3(0.1373, 0.1333, 0.1255);
    vec3 c2 = vec3(0.6000, 0.5961, 0.5686);
    vec3 c3 = vec3(0.4980, 0.4863, 0.4706);
    vec3 c4 = vec3(0.4039, 0.3843, 0.3647);
    vec3 c5 = vec3(0.3255, 0.2706, 0.2471);
    ...
    float edge = length(photo - photoSoft) * 0.5871;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4084) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6394);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2301, 0.0, 0.42));
    col += c5 * bead * 0.1050;
