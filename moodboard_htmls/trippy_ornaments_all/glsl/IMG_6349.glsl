...
void main() {
    ...
    float petals = cos(ma * 24.0000 + r * 2.2500 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.8557) *
                    sin((k.x - k.y) * 27.1266);
    float bead = smoothstep(0.86, 0.99, cos(ma * 14.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.8286 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.6025 + t * 1.8 + petals),
        cos(k.x * 10.2387 - t * 1.6 + ringWave)
    ) * 0.0153;
    ...
    vec3 c1 = vec3(0.1255, 0.1098, 0.1294);
    vec3 c2 = vec3(0.4863, 0.3922, 0.3961);
    vec3 c3 = vec3(0.6745, 0.1020, 0.1216);
    vec3 c4 = vec3(0.8549, 0.8549, 0.8275);
    vec3 c5 = vec3(0.6902, 0.6745, 0.6588);
    ...
    float edge = length(photo - photoSoft) * 0.6547;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3771) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6936);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2646, 0.0, 0.42));
    col += c5 * bead * 0.1223;
