...
void main() {
    ...
    float petals = cos(ma * 16.0000 + r * 6.6184 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.1656) *
                    sin((k.x - k.y) * 23.6736);
    float bead = smoothstep(0.86, 0.99, cos(ma * 16.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.8417 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.2958 + t * 1.8 + petals),
        cos(k.x * 8.9233 - t * 1.6 + ringWave)
    ) * 0.0124;
    ...
    vec3 c1 = vec3(0.1686, 0.1490, 0.1882);
    vec3 c2 = vec3(0.5059, 0.5529, 0.6196);
    vec3 c3 = vec3(0.3961, 0.4314, 0.5255);
    vec3 c4 = vec3(0.3333, 0.3098, 0.3294);
    vec3 c5 = vec3(0.2235, 0.2902, 0.4235);
    ...
    float edge = length(photo - photoSoft) * 0.5601;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3789) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.5795);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2546, 0.0, 0.42));
    col += c5 * bead * 0.1173;
