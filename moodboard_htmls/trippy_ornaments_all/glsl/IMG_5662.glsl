...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 6.2500 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.3321) *
                    sin((k.x - k.y) * 25.2472);
    float bead = smoothstep(0.86, 0.99, cos(ma * 16.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.4856 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.4809 + t * 1.8 + petals),
        cos(k.x * 9.5227 - t * 1.6 + ringWave)
    ) * 0.0137;
    ...
    vec3 c1 = vec3(0.4275, 0.4157, 0.3804);
    vec3 c2 = vec3(0.5686, 0.5451, 0.4980);
    vec3 c3 = vec3(0.7020, 0.6784, 0.6314);
    vec3 c4 = vec3(0.8196, 0.8078, 0.7882);
    vec3 c5 = vec3(0.5020, 0.2510, 0.2392);
    ...
    float edge = length(photo - photoSoft) * 0.6032;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4647) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6611);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2281, 0.0, 0.42));
    col += c5 * bead * 0.1041;
