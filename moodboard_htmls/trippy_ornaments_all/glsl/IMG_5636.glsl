...
void main() {
    ...
    float ringWave = sin(r * 17.0000 - t * 3.0);
    float petals = cos(ma * 18.0000 + r * 5.1447 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.8796) *
                    sin((k.x - k.y) * 24.0214);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.6468 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.7243 + t * 1.8 + petals),
        cos(k.x * 9.0558 - t * 1.6 + ringWave)
    ) * 0.0127;
    ...
    vec3 c1 = vec3(0.1255, 0.1020, 0.1176);
    vec3 c2 = vec3(0.4588, 0.4392, 0.4157);
    vec3 c3 = vec3(0.5725, 0.5412, 0.4745);
    vec3 c4 = vec3(0.3255, 0.2000, 0.2078);
    vec3 c5 = vec3(0.3216, 0.2863, 0.3098);
    ...
    float edge = length(photo - photoSoft) * 0.5696;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3529) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6518);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2360, 0.0, 0.42));
    col += c5 * bead * 0.1080;
