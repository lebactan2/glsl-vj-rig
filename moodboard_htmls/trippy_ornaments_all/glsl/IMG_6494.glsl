...
void main() {
    ...
    float petals = cos(ma * 16.0000 + r * 4.6184 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 19.6022) *
                    sin((k.x - k.y) * 25.7128);
    float bead = smoothstep(0.86, 0.99, cos(ma * 20.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.9236 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 7.7121 + t * 1.8 + petals),
        cos(k.x * 9.7001 - t * 1.6 + ringWave)
    ) * 0.0141;
    ...
    vec3 c1 = vec3(0.6784, 0.6627, 0.6118);
    vec3 c2 = vec3(0.3843, 0.3804, 0.3686);
    vec3 c3 = vec3(0.7255, 0.7137, 0.6745);
    vec3 c4 = vec3(0.5216, 0.5176, 0.4941);
    vec3 c5 = vec3(0.8353, 0.8392, 0.8196);
    ...
    float edge = length(photo - photoSoft) * 0.6159;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5231) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6389);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2031, 0.0, 0.42));
    col += c5 * bead * 0.0916;
