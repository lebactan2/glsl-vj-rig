...
void main() {
    ...
    float petals = cos(ma * 22.0000 + r * 3.7237 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.2885) *
                    sin((k.x - k.y) * 24.7177);
    float bead = smoothstep(0.86, 0.99, cos(ma * 17.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.5161 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.3505 + t * 1.8 + petals),
        cos(k.x * 9.3210 - t * 1.6 + ringWave)
    ) * 0.0133;
    ...
    vec3 c1 = vec3(0.1255, 0.1059, 0.0941);
    vec3 c2 = vec3(0.4078, 0.3725, 0.3961);
    vec3 c3 = vec3(0.5137, 0.4902, 0.4667);
    vec3 c4 = vec3(0.2078, 0.1765, 0.1490);
    vec3 c5 = vec3(0.3647, 0.2471, 0.2431);
    ...
    float edge = length(photo - photoSoft) * 0.5887;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3355) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6487);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2564, 0.0, 0.42));
    col += c5 * bead * 0.1182;
