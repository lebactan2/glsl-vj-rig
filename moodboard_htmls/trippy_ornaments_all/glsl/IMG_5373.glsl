...
void main() {
    ...
    float ringWave = sin(r * 11.4000 - t * 3.0);
    float petals = cos(ma * 12.0000 + r * 6.9342 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.0639) *
                    sin((k.x - k.y) * 27.7459);
    float bead = smoothstep(0.86, 0.99, cos(ma * 20.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.6251 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.8062 + t * 1.8 + petals),
        cos(k.x * 10.4746 - t * 1.6 + ringWave)
    ) * 0.0158;
    ...
    vec3 c1 = vec3(0.1843, 0.1255, 0.1451);
    vec3 c2 = vec3(0.8392, 0.8196, 0.7725);
    vec3 c3 = vec3(0.3412, 0.2863, 0.3255);
    vec3 c4 = vec3(0.4549, 0.3373, 0.3490);
    vec3 c5 = vec3(0.6118, 0.5882, 0.5961);
    ...
    float edge = length(photo - photoSoft) * 0.6716;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4833) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6455);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2387, 0.0, 0.42));
    col += c5 * bead * 0.1094;
