...
void main() {
    ...
    float ringWave = sin(r * 22.6000 - t * 3.0);
    float petals = cos(ma * 16.0000 + r * 3.3026 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 19.4713) *
                    sin((k.x - k.y) * 23.8860);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.7901 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 7.6539 + t * 1.8 + petals),
        cos(k.x * 9.0042 - t * 1.6 + ringWave)
    ) * 0.0126;
    ...
    vec3 c1 = vec3(0.4431, 0.4275, 0.4196);
    vec3 c2 = vec3(0.4118, 0.4039, 0.3961);
    vec3 c3 = vec3(0.7686, 0.7373, 0.6706);
    vec3 c4 = vec3(0.5373, 0.5216, 0.4980);
    vec3 c5 = vec3(0.8314, 0.8039, 0.7529);
    ...
    float edge = length(photo - photoSoft) * 0.5659;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5053) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6445);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2013, 0.0, 0.42));
    col += c5 * bead * 0.0906;
