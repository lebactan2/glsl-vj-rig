...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 3.1447 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.3826) *
                    sin((k.x - k.y) * 26.0392);
    float bead = smoothstep(0.86, 0.99, cos(ma * 15.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.0970 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.9478 + t * 1.8 + petals),
        cos(k.x * 9.8244 - t * 1.6 + ringWave)
    ) * 0.0144;
    ...
    vec3 c1 = vec3(0.3020, 0.2510, 0.2196);
    vec3 c2 = vec3(0.2275, 0.1804, 0.1490);
    vec3 c3 = vec3(0.2627, 0.2275, 0.2000);
    vec3 c4 = vec3(0.4000, 0.4392, 0.4314);
    vec3 c5 = vec3(0.6353, 0.8667, 0.8902);
    ...
    float edge = length(photo - photoSoft) * 0.6249;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4129) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6167);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2433, 0.0, 0.42));
    col += c5 * bead * 0.1117;
