...
void main() {
    ...
    float petals = cos(ma * 24.0000 + r * 5.8289 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 25.1435) *
                    sin((k.x - k.y) * 23.1652);
    float bead = smoothstep(0.86, 0.99, cos(ma * 13.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.6757 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 10.1749 + t * 1.8 + petals),
        cos(k.x * 8.7296 - t * 1.6 + ringWave)
    ) * 0.0120;
    ...
    vec3 c1 = vec3(0.1961, 0.1725, 0.1608);
    vec3 c2 = vec3(0.3569, 0.3765, 0.3490);
    vec3 c3 = vec3(0.4745, 0.2000, 0.1804);
    vec3 c4 = vec3(0.5686, 0.5098, 0.4471);
    vec3 c5 = vec3(0.6000, 0.4039, 0.3255);
    ...
    float edge = length(photo - photoSoft) * 0.5462;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3568) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6995);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2832, 0.0, 0.42));
    col += c5 * bead * 0.1316;
