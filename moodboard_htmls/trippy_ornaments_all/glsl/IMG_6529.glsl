...
void main() {
    ...
    float petals = cos(ma * 18.0000 + r * 6.3026 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.0499) *
                    sin((k.x - k.y) * 24.8860);
    float bead = smoothstep(0.86, 0.99, cos(ma * 26.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.6236 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.8000 + t * 1.8 + petals),
        cos(k.x * 9.3851 - t * 1.6 + ringWave)
    ) * 0.0134;
    ...
    vec3 c1 = vec3(0.5961, 0.5843, 0.5020);
    vec3 c2 = vec3(0.2471, 0.2078, 0.2000);
    vec3 c3 = vec3(0.4824, 0.4118, 0.3451);
    vec3 c4 = vec3(0.2863, 0.3412, 0.3882);
    vec3 c5 = vec3(0.8118, 0.8118, 0.7529);
    ...
    float edge = length(photo - photoSoft) * 0.5933;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4831) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6560);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2385, 0.0, 0.42));
    col += c5 * bead * 0.1092;
