...
void main() {
    ...
    float petals = cos(ma * 12.0000 + r * 3.3026 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.2739) *
                    sin((k.x - k.y) * 23.6973);
    float bead = smoothstep(0.86, 0.99, cos(ma * 13.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.6327 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.3439 + t * 1.8 + petals),
        cos(k.x * 8.9323 - t * 1.6 + ringWave)
    ) * 0.0124;
    ...
    vec3 c1 = vec3(0.2588, 0.2784, 0.2471);
    vec3 c2 = vec3(0.5451, 0.5294, 0.4627);
    vec3 c3 = vec3(0.5020, 0.4157, 0.3686);
    vec3 c4 = vec3(0.6745, 0.6627, 0.5412);
    vec3 c5 = vec3(0.5804, 0.3059, 0.3059);
    ...
    float edge = length(photo - photoSoft) * 0.5608;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4844) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6772);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2562, 0.0, 0.42));
    col += c5 * bead * 0.1181;
