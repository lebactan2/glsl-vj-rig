...
void main() {
    ...
    float petals = cos(ma * 22.0000 + r * 6.3553 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.4053) *
                    sin((k.x - k.y) * 23.4368);
    float bead = smoothstep(0.86, 0.99, cos(ma * 17.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 5.8991 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.9579 + t * 1.8 + petals),
        cos(k.x * 8.8331 - t * 1.6 + ringWave)
    ) * 0.0122;
    ...
    vec3 c1 = vec3(0.0863, 0.0863, 0.0706);
    vec3 c2 = vec3(0.1373, 0.1294, 0.1098);
    vec3 c3 = vec3(0.2980, 0.2627, 0.2196);
    vec3 c4 = vec3(0.4471, 0.3922, 0.3255);
    vec3 c5 = vec3(0.2157, 0.1647, 0.1412);
    ...
    float edge = length(photo - photoSoft) * 0.5536;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.2532) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6619);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2436, 0.0, 0.42));
    col += c5 * bead * 0.1118;
