...
void main() {
    ...
    float petals = cos(ma * 18.0000 + r * 7.0921 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.7191) *
                    sin((k.x - k.y) * 24.4068);
    float bead = smoothstep(0.86, 0.99, cos(ma * 19.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 8.0753 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.6529 + t * 1.8 + petals),
        cos(k.x * 9.2026 - t * 1.6 + ringWave)
    ) * 0.0130;
    ...
    vec3 c1 = vec3(0.2314, 0.3216, 0.2784);
    vec3 c2 = vec3(0.6275, 0.6431, 0.6471);
    vec3 c3 = vec3(0.7059, 0.7725, 0.8510);
    vec3 c4 = vec3(0.4784, 0.4745, 0.4314);
    vec3 c5 = vec3(0.7882, 0.8431, 0.8980);
    ...
    float edge = length(photo - photoSoft) * 0.5802;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5434) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.5994);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2337, 0.0, 0.42));
    col += c5 * bead * 0.1069;
