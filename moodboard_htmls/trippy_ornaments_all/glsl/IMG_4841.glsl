...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 5.7763 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 20.4437) *
                    sin((k.x - k.y) * 26.4026);
    float bead = smoothstep(0.86, 0.99, cos(ma * 17.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.9795 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.0861 + t * 1.8 + petals),
        cos(k.x * 9.9629 - t * 1.6 + ringWave)
    ) * 0.0147;
    ...
    vec3 c1 = vec3(0.1882, 0.1961, 0.2235);
    vec3 c2 = vec3(0.7922, 0.8275, 0.7529);
    vec3 c3 = vec3(0.3961, 0.3922, 0.4118);
    vec3 c4 = vec3(0.4980, 0.4745, 0.4863);
    vec3 c5 = vec3(0.8157, 0.8902, 0.8196);
    ...
    float edge = length(photo - photoSoft) * 0.6348;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5306) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6233);
