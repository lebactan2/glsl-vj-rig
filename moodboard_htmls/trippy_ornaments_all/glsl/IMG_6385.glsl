...
void main() {
    ...
    float petals = cos(ma * 24.0000 + r * 7.1447 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.7674) *
                    sin((k.x - k.y) * 24.0117);
    float bead = smoothstep(0.86, 0.99, cos(ma * 15.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.4144 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.5633 + t * 1.8 + petals),
        cos(k.x * 9.0521 - t * 1.6 + ringWave)
    ) * 0.0127;
    ...
    vec3 c1 = vec3(0.5608, 0.4941, 0.3843);
    vec3 c2 = vec3(0.2314, 0.2157, 0.1725);
    vec3 c3 = vec3(0.3804, 0.3804, 0.3137);
    vec3 c4 = vec3(0.6784, 0.5961, 0.3843);
    vec3 c5 = vec3(0.5333, 0.4078, 0.2784);
    ...
    float edge = length(photo - photoSoft) * 0.5694;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4553) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.7028);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2633, 0.0, 0.42));
    col += c5 * bead * 0.1217;
