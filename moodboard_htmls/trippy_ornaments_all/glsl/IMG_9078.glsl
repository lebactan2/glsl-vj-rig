...
void main() {
    ...
    float petals = cos(ma * 12.0000 + r * 6.7763 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.0464) *
                    sin((k.x - k.y) * 24.4721);
    float bead = smoothstep(0.86, 0.99, cos(ma * 24.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 6.8998 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.7984 + t * 1.8 + petals),
        cos(k.x * 9.2275 - t * 1.6 + ringWave)
    ) * 0.0131;
    ...
    vec3 c1 = vec3(0.2941, 0.2941, 0.3294);
    vec3 c2 = vec3(0.1725, 0.1804, 0.2314);
    vec3 c3 = vec3(0.4118, 0.3922, 0.3922);
    vec3 c4 = vec3(0.6275, 0.5922, 0.5412);
    vec3 c5 = vec3(0.8078, 0.7686, 0.6941);
    ...
    float edge = length(photo - photoSoft) * 0.5820;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.3866) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6350);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2384, 0.0, 0.42));
    col += c5 * bead * 0.1092;
