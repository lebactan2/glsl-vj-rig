...
void main() {
    ...
    float petals = cos(ma * 16.0000 + r * 5.6711 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 22.0470) *
                    sin((k.x - k.y) * 27.4303);
    float bead = smoothstep(0.86, 0.99, cos(ma * 25.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.4584 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.7987 + t * 1.8 + petals),
        cos(k.x * 10.3544 - t * 1.6 + ringWave)
    ) * 0.0155;
    ...
    vec3 c1 = vec3(0.0706, 0.0980, 0.1412);
    vec3 c2 = vec3(0.7412, 0.7373, 0.7216);
    vec3 c3 = vec3(0.2588, 0.3176, 0.3490);
    vec3 c4 = vec3(0.4157, 0.4078, 0.4275);
    vec3 c5 = vec3(0.8510, 0.8667, 0.8667);
    ...
    float edge = length(photo - photoSoft) * 0.6630;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4611) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6094);
