...
void main() {
    ...
    float petals = cos(ma * 20.0000 + r * 4.4605 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 20.2636) *
                    sin((k.x - k.y) * 25.5012);
    float bead = smoothstep(0.86, 0.99, cos(ma * 16.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 8.1584 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.0060 + t * 1.8 + petals),
        cos(k.x * 9.6195 - t * 1.6 + ringWave)
    ) * 0.0139;
    ...
    vec3 c1 = vec3(0.2510, 0.2471, 0.2196);
    vec3 c2 = vec3(0.8039, 0.7922, 0.7686);
    vec3 c3 = vec3(0.5490, 0.5216, 0.4745);
    vec3 c4 = vec3(0.4314, 0.4471, 0.4353);
    vec3 c5 = vec3(0.6784, 0.6471, 0.6039);
    ...
    float edge = length(photo - photoSoft) * 0.6102;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.5544) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6457);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2127, 0.0, 0.42));
    col += c5 * bead * 0.0963;
