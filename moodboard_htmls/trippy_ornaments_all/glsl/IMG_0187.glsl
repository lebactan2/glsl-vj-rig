...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 4.7237 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 21.3668) *
                    sin((k.x - k.y) * 22.6161);
    float bead = smoothstep(0.86, 0.99, cos(ma * 27.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.0502 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.4964 + t * 1.8 + petals),
        cos(k.x * 8.5204 - t * 1.6 + ringWave)
    ) * 0.0115;
    ...
    vec3 c1 = vec3(0.2275, 0.2157, 0.1961);
    vec3 c2 = vec3(0.3922, 0.3490, 0.3529);
    vec3 c3 = vec3(0.4471, 0.4275, 0.4157);
    vec3 c4 = vec3(0.5412, 0.5020, 0.4863);
    vec3 c5 = vec3(0.5059, 0.4549, 0.4275);
    ...
    float edge = length(photo - photoSoft) * 0.5312;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4067) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6507);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2286, 0.0, 0.42));
    col += c5 * bead * 0.1043;
