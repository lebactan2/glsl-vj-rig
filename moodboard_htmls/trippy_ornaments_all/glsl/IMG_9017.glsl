...
void main() {
    ...
    float petals = cos(ma * 14.0000 + r * 3.3026 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 20.4399) *
                    sin((k.x - k.y) * 25.0250);
    float bead = smoothstep(0.86, 0.99, cos(ma * 17.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.3618 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 8.0844 + t * 1.8 + petals),
        cos(k.x * 9.4381 - t * 1.6 + ringWave)
    ) * 0.0135;
    ...
    vec3 c1 = vec3(0.1490, 0.1569, 0.1490);
    vec3 c2 = vec3(0.6431, 0.6667, 0.6392);
    vec3 c3 = vec3(0.4549, 0.4078, 0.3804);
    vec3 c4 = vec3(0.5686, 0.5647, 0.5373);
    vec3 c5 = vec3(0.3255, 0.3216, 0.2980);
    ...
    float edge = length(photo - photoSoft) * 0.5971;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4482) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6347);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2152, 0.0, 0.42));
    col += c5 * bead * 0.0976;
