...
void main() {
    ...
    float petals = cos(ma * 22.0000 + r * 6.3026 + sin(t + r * 2.0));
    float lattice = sin((k.x + k.y) * 23.1784) *
                    sin((k.x - k.y) * 24.7232);
    float bead = smoothstep(0.86, 0.99, cos(ma * 23.0000 - t * 2.4)) *
                 smoothstep(0.035, 0.0, abs(fract(r * 7.3275 - t * 0.18) - 0.5));

    vec2 warp = vec2(
        sin(k.y * 9.3015 + t * 1.8 + petals),
        cos(k.x * 9.3231 - t * 1.6 + ringWave)
    ) * 0.0133;
    ...
    vec3 c1 = vec3(0.4314, 0.3451, 0.3059);
    vec3 c2 = vec3(0.5529, 0.4431, 0.3804);
    vec3 c3 = vec3(0.4902, 0.3647, 0.3098);
    vec3 c4 = vec3(0.8275, 0.8196, 0.7843);
    vec3 c5 = vec3(0.2471, 0.1922, 0.1843);
    ...
    float edge = length(photo - photoSoft) * 0.5888;
    mask *= smoothstep(1.28, 0.02, r);
    mask *= 0.65 + smoothstep(0.05, 0.38, edge + abs(luma(photo) - 0.4437) * 0.35);

    vec3 tintedPhoto = mix(photo.bgr, photo, 0.6815);
    vec3 col = mix(tintedPhoto, patternColor, clamp(mask * 0.2548, 0.0, 0.42));
    col += c5 * bead * 0.1174;
