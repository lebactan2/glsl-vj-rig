void main() {
    ...
    // Red tile wall
        ...
        if (fract(tileP.y) > 0.5) tileP.x += 0.5; // Offset tiles
        
        vec2 grid = fract(tileP);
        if (grid.x < 0.02 || grid.y < 0.02) col = vec3(0.6, 0.5, 0.5); // Mortar
        
        // Lighting gradient on wall
        col *= 0.5 + 0.5 * smoothstep(1.5, 0.0, length(p));
        if (p.x > 0.8) col *= 0.6; // Corner shadow
    }
    ...
    if (lDist < lanternRadius) {
        ...
        // Scrolling/Wrapping Characters (Animated rotation)
        float wrapX = atan(lP.x, z) * 2.0 + iTime*0.5; // Slowly rotating
        ...
        if (lP.y < -0.35) {
            col = vec3(0.8, 0.7, 0.2); // Yellow/gold
            float scallop = sin(wrapX * 20.0);
            if (lP.y < -0.4 + scallop * 0.02) {
                col = vec3(0.9, 0.3, 0.2); // Red border
                // Floral details
                if (fract(wrapX * 8.0) < 0.2) col = vec3(0.3, 0.6, 0.3); // Green leaves
            }
        }
    }
    ...
    if (tP.x > -0.1 && tP.x < 0.1 && tP.y < -0.55 && tP.y > -0.85) {
        col = vec3(0.6, 0.2, 0.15); // Red fringe
        
        // Tassel strands swaying
        float strandSway = sin(iTime * 3.0 + tP.y * 10.0) * 0.02;
        if (fract((tP.x + strandSway) * 100.0) < 0.3) col *= 0.7; // Shadows between strands
