/* @layer_metadata
{
  "title": "Shader: IMG_7211",
  "layers": [
    {
      "name": "Background",
      "keywords": ["background", "dark"]
    },
    {
      "name": "Top Bar",
      "keywords": ["top", "bar", "status", "static"]
    },
    {
      "name": "Bottom Bar",
      "keywords": ["bottom", "bar", "static"]
    },
    {
      "name": "Scroll Content",
      "keywords": ["scroll", "content", "header", "menu", "tabs", "ads", "cards", "book"]
    }
  ]
}
*/
void layer_Background(in vec2 p, inout vec3 col, in vec3 bgDark) {
    col = bgDark;
}

void layer_TopBar(in vec2 p, inout vec3 col) {
    if (p.y > 0.9) {
...
void layer_BottomBar(in vec2 p, inout vec3 col) {
    if (p.y <= -0.5) {
...
void layer_ScrollContent(in vec2 p, in float iTime, inout vec3 col, in vec3 bgDark) {
    if (p.y <= 0.9 && p.y > -0.5) {
        float scrollY = p.y + fract(iTime * 0.2) * 1.0;
        float cy = mod(scrollY, 1.5) - 0.75; 
