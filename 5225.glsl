void main() {
    ...
    if (p.y > -0.5 && p.y < 0.4) {
        ...
        if (imgP.y > 0.0) {
            // Animated perspective dome
            float perspective = 1.0 - imgP.y * 0.5;
            float shift = sin(iTime * 0.5) * 0.1; // Shift grid
        ...
        if (imgP.x > -1.0 && imgP.x < 0.1 && imgP.y < -0.4) {
            ...
            if (personIdx >= 0.0 && personIdx <= 7.0) {
                ...
                // Animated bounce offset per person
