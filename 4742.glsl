void main() {
    ...
    // Animated scroll for street to simulate walking
    vec2 sp = p;
    sp.y += iTime * 0.5;
    
    // Street background
    vec3 col = vec3(0.3, 0.3, 0.32);
    // Noise on street
    float noise = fract(sin(dot(sp * 100.0, vec2(12.9898, 78.233))) * 43758.5453);
    col += (noise - 0.5) * 0.05;

    // Person Center (bobbing up and down)
    ...
    // Clothes Pattern
    vec3 clothesBase = vec3(0.05, 0.1, 0.15); // Dark blue/black
    vec3 dotColor = vec3(0.2, 0.8, 0.3); // Green dots
    ...
    // Hat (Conical, green with black dots)
    vec2 hp = p - vec2(-0.5, 0.0 + bob);
    // slight rotation to hat
    ...
    if (hat < 0.0) {
        col = vec3(0.3, 0.9, 0.1); // Bright green
        // Black dots
    ...
    // Motorcycle bottom left (stationary relative to street)
