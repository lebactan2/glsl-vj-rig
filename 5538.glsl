void main() {
    ...
    // --- 1. Motorcycle Wheel (Right Side) Animated ---
    ...
    if (wheelR < 1.3) {
        ...
        if (wheelR > 1.0) {
            float angle = atan(p.y - wheelCenter.y, p.x - wheelCenter.x) + iTime * 2.0; // Rotating tire
            ...
            float angle = atan(p.y - wheelCenter.y, p.x - wheelCenter.x) + iTime * 2.0; // Rotating spokes
