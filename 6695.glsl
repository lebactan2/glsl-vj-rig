void main() {
    ...
    if (p.y < 0.4 && p.y > -0.2 && abs(p.x) < torsoWidth) {
        ...
        if (p.y > 0.15 && p.y < 0.22) {
            col = vec3(0.7); 
            // Shiny metallic sweep on reflective band
    ...
    if (dArmLeft < 0.0 && p.y < 0.3 && !isFigure) {
        col = uniformBlue;
        isFigure = true;
    ...
    if (dArmRight < 0.0 && p.y < 0.3 && !isFigure) {
        col = uniformBlue;
        isFigure = true;
        if (p.y < -0.22) col = accentOrange;
        col *= 0.9;
    ...
    if (length(p - vec2(-0.4, -0.35)) < 0.05) { col = skinTone; isFigure = true; }
    if (length(p - vec2(0.4, -0.35)) < 0.05) { col = skinTone; isFigure = true; }
