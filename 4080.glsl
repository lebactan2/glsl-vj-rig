void main() {
    ...
    // Base fabric: bright yellow-green
    vec3 col = vec3(0.8, 0.9, 0.2);
    
    // Fabric wrinkles, enhanced by animation
    ...
    // Top symbol (square with circle/star)
    ...
    // Top right wood slats
    if (p.x > 0.4 && p.y > 0.5) {
        float slat = fract(p.x * 4.0 - p.y * 2.0);
        if (slat > 0.1) {
            col = vec3(0.8, 0.5, 0.3) * (0.8 + 0.2*sin(p.x*50.0)); // Wood color
        }
    }
    
    // Right grey ground
    if (p.x > 0.6) {
        // Non-wavy p for ground
        vec2 gp = uv * 2.0 - 1.0; gp.x *= iResolution.x / iResolution.y;
