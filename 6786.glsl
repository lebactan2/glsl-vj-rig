void main() {
    ...
    col *= 0.8 + 0.2 * p.y; // lighting

    // --- Ticket ---
    float tW = 1.3;
    float tH = 0.45;
    
    // Ticket slight fold/bend + floating animation
        ...
        if (tp.x > -1.0 && tp.x < 1.25 && tp.y > -0.4 && tp.y < 0.4) {
            ...
            if (gx < 0.02 || gy < 0.03) {
                ...
                if (tp.x > -0.8) {
                    ...
                    // Animated numbers flashing/changing
