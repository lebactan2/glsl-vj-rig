void main() {
    ...
    // Background: light grey/white floor and bright background
    vec3 col = vec3(0.85, 0.85, 0.85);
    
    // Add some noise to the background floor
    float bgNoise = fract(sin(dot(p + iTime*0.05, vec2(12.9898, 78.233))) * 43758.5453);
    if (p.x + p.y > 0.5) {
        col *= 0.95 + 0.05 * bgNoise; // Floor texture
    } else {
        col = vec3(0.95); // Brighter top left
    }

    // --- The Rock Speaker ---
    
    // Base shape of the rock
    // Distorted circle
    float a = atan(p.y, p.x);
    float r = length(p);
    
    // Low frequency distortion for the overall shape
    float rockShape = 0.65;
    rockShape += sin(a * 3.0 + iTime*0.2) * 0.1; // Gentle breathing animation
    rockShape += cos(a * 5.0 + 1.0) * 0.05;
    rockShape += sin(a * 7.0) * 0.03;
    
    // High frequency noise for the bumpy edge
    float edgeNoise = fract(sin(a * 50.0) * 4375.5453) * 0.02;
    rockShape += edgeNoise;

    // Shift rock center slightly
    vec2 rp = p - vec2(-0.1, -0.1);
    float rLength = length(rp);
    
    if (rLength < rockShape) {
        // Rock Base Color (Dark Grey)
        ...
        if (dHoles < 0.0) {
            ...
            // Subtle mesh pattern animation (pulsing to "music")
